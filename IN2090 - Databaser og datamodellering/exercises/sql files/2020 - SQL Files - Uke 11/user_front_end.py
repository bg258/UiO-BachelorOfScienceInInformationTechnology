...
def register(conn):
    ...
    cur = conn.cursor() # Create a curstor object that can be used for executing queries
    # We can use %s as a place holder for a value, and the pass a tuple of value
    # to be submitted for these place holders. The first placeholder is the substituted
    # with the first element in the tuple, and so on.
...
def login(conn):
    ...
    # injection attacks. More on this next lecture.
    ...
    # cursor. This will make a lists, where each inner list represents one row
    rows = cur.fetchall()

    if (rows == []):
        # The query returned no results, this the user-password pair does not
        # exist in the DB
...
def search(conn, username):
    ...
    # For long queries, it is helpful to name the placeholders
    # This is done by placing the name of the place holder in parenthesis
    ...
    rows = cur.fetchall() # Retrieve all results into a list of tuples
...
def get_int_from_user(msg, needed):
    # Utility method that gets an int fromm the user with the first argument as
